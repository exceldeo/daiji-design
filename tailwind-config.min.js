tailwind.config = (function () {
  ...
  const blackColorStpe = [
    { number: 50, hexcode: "#f6f6f6" },
    { number: 100, hexcode: "#e7e7e7" },
    { number: 200, hexcode: "#d1d1d1" },
    { number: 300, hexcode: "#b0b0b0" },
    { number: 400, hexcode: "#888888" },
    { number: 500, hexcode: "#6d6d6d" },
    { number: 600, hexcode: "#5d5d5d" },
    { number: 700, hexcode: "#4f4f4f" },
    { number: 800, hexcode: "#454545" },
    { number: 900, hexcode: "#3d3d3d" },
    { number: 950, hexcode: "#000000" },
  ];
  const whiteColorStpe = [
    { number: 50, hexcode: "#ffffff" },
    { number: 100, hexcode: "#efefef" },
    { number: 200, hexcode: "#dcdcdc" },
    { number: 300, hexcode: "#bdbdbd" },
    { number: 400, hexcode: "#989898" },
    { number: 500, hexcode: "#7c7c7c" },
    { number: 600, hexcode: "#656565" },
    { number: 700, hexcode: "#525252" },
    { number: 800, hexcode: "#464646" },
    { number: 900, hexcode: "#3d3d3d" },
    { number: 950, hexcode: "#292929" },
  ];
  const colorSteps = [
    { name: "Slate", id: "slate", shades: slateColorStep },
    { name: "Gray", id: "gray", shades: grayColorStep },
    { name: "Zinc", id: "zinc", shades: zincColorStep },
    { name: "Neutral", id: "neutral", shades: neutralColorStep },
    { name: "Stone", id: "stone", shades: stoneColorStep },
    { name: "Red", id: "red", shades: redColorStep },
    { name: "Orange", id: "orange", shades: orangeColorStep },
    { name: "Amber", id: "amber", shades: amberColorStep },
    { name: "Yellow", id: "yellow", shades: yellowColorStep },
    { name: "Lime", id: "lime", shades: limeColorStep },
    { name: "Green", id: "green", shades: greenColorStep },
    { name: "Emerald", id: "emerald", shades: emeraldColorStep },
    { name: "Teal", id: "teal", shades: tealColorStep },
    { name: "Cyan", id: "cyan", shades: cyanColorStep },
    { name: "Sky", id: "sky", shades: skyColorStep },
    { name: "Blue", id: "blue", shades: blueColorStep },
    { name: "Indigo", id: "indigo", shades: indigoColorStep },
    { name: "Violet", id: "violet", shades: violetColorStep },
    { name: "Purple", id: "purple", shades: purpleColorStep },
    { name: "Fuchsia", id: "fuchsia", shades: fuchsiaColorStep },
    { name: "Pink", id: "pink", shades: pinkColorStep },
    { name: "Rose", id: "rose", shades: roseColorStep },
    { name: "Black", id: "black", shades: blackColorStpe },
    { name: "White", id: "white", shades: whiteColorStpe },
  ];
  const k = 29 ** 3 / 3 ** 3;
  const e = 6 ** 3 / 29 ** 3;
  ...
  function convertXyz50ToLab(xyz) {
    ...
    const f1 = f(y / D50.Y);
    const f2 = f(z / D50.Z);
    return { L: 116 * f1 - 16, a: 500 * (f0 - f1), b: 200 * (f1 - f2) };
  }
  ...
  function convertIntRgbToFloatRgb(rgb) {
    return { r: rgb.r / 255, g: rgb.g / 255, b: rgb.b / 255, a: rgb.a ?? 1 };
  }
  function hexToRgb(hex) {
    const result =
      /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})?$/i.exec(hex);
    if (!result) throw Error("hex éŠé—´ç¬‰éšå Ÿç¡¶");
  ...
  function calculateDeltaE2000(Lab1, Lab2) {
    ...
    const ac1c2 = (c1 + c2) / 2;
    const g = 0.5 * (1 - Math.sqrt(ac1c2 ** 7 / (ac1c2 ** 7 + 25 ** 7)));
    const a1p = (1 + g) * a1;
    const a2p = (1 + g) * a2;
  ...
  function getSimilarColorAndColorStepByHex(hex) {
    ...
    for (let i = 0; i < colorSteps.length; i++) {
      ...
      for (let j = 0; j < colorStep.length; j++) {
        ...
        const lab1 = convertRgbToLab(
          convertIntRgbToFloatRgb(hexToRgb(shade.hexcode))
        );
        const similarNum = calculateDeltaE2000(
    ...
    return { similarColorStep: similarColorStep };
  }
  const borderRadiusMap = { small: "4px", medium: "8px", large: "9999px" };
  const script = document.currentScript;
  const primaryDefault = script.getAttribute("data-color");
  const borderRadius =
    borderRadiusMap[script.getAttribute("data-border-radius")] ?? {};
  const primary = {};
  if (primaryDefault) {
    const { similarColorStep } =
      getSimilarColorAndColorStepByHex(primaryDefault);
...
window.handleAIError = () => {
  ...
  const forms = inputButtonComponent.filter(
    (form) =>
      form.querySelectorAll("input").length === 1 &&
      form.querySelectorAll("button").length === 1
  );
  forms.forEach((form) => {
    const input = form.querySelector("input");
    const button = form.querySelector("button");
    const isComponent = Array.from(input.classList).some((item) =>
      item.includes("rounded-l")
    );
    if (isComponent) {
      ...
      const divs = Array.from(form.querySelectorAll("div")).filter(
        (div) =>
  ...
  const divAComponent = divs.filter((div) => {
    ...
    return (
      children.length === 1 &&
      (children[0].tagName.toLowerCase() === "button" ||
